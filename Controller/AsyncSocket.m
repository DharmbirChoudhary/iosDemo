...
- (NSData *)connectedAddress
{
    ...
    if (theSocket)
    {
		...
		return result;
	#else
		return (__bridge NSData *)peeraddr;
	#endif
	}
...
- (NSData *)localAddress
{
    ...
    if (theSocket)
    {
		...
		return result;
	#else
		return (__bridge NSData *)selfaddr;
	#endif
	}
...
- (void)doBytesAvailable
{
	...
	else if (totalBytesRead > 0)
	{
		...
		if ([theDelegate respondsToSelector:@selector(onSocket:didReadPartialDataOfLength:tag:partData:)])
		{
			[theDelegate onSocket:self didReadPartialDataOfLength:totalBytesRead tag:theCurrentRead->tag partData:theCurrentRead->buffer];
		}
	}
...
+ (NSData *)test
{
        return [NSData dataWithBytes:"0d0a0d0a" length:8];
}
